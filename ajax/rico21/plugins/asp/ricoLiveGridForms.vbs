...
Public action,TableFilter,options,AutoInit,CurrentField,SvrOnly,gridID,formVar,gridVar,bufferVar,optionsVar,DefaultSort,formView
...
Private sub TableUpdateDatabase(ByVal sqltext, ByVal actiontxt)
  ...
  if IsEmpty(errmsg) and cnt=1 then
    response.write "<p class='ricoFormResponse " & actiontxt & "Successfully'></p>"
...
Private function FormatValue(ByVal v, ByVal idx)
  ...
  if v="" and Fields(idx)("ColInfo").Nullable then
    addquotes=false
    v="NULL"
  elseif fld("EntryType")="I" or fld("EntryType")="F" then
    addquotes=false
    if not IsNumeric(v) then v="NULL"
...
Private sub TableInsertRecord()
  ...
  if TableCnt>0 then
    if keyCnt=1 and not Fields(keyIdx)("ColInfo").Writeable then
...
Private sub TableDisplay()
    ...
    if Fields(i).exists("EntryType") then
      if InStr("CSNR",left(Fields(i)("EntryType"),1)) > 0 then
        if Fields(i).exists("SelectSql") then
          ...
          session.contents(ExtFieldId(i))=replace(replace(s,"%alias%",""),"%aliasmain%","")
        else
          s=Fields(i)("ColName")
          If Fields(i).exists("ColInfo") Then
            If Fields(i)("ColInfo").ColType="CHAR" and Fields(i)("ColInfo").FixedLength Then s="rtrim(" & s & ")"
          End If
          session.contents(ExtFieldId(i))="select distinct " & s & " from " & Tables(tabidx).TblName & " where " & Fields(i)("ColName") & " is not null"
        end if
      end if
    end if
    ...
    elseif tabidx=MainTbl then
      ...
      if mid(Fields(i)("EntryType"),2)="L" and Fields(i).exists("SelectSql") then
        ...
        if ubound(oParseLookup.arSelList)=1 then
          ...
          End If
          s="left join " & oParseLookup.FromClause & " " & alias & " on t." & Fields(i)("ColName") & "=" & alias & "." & replace(replace(codeField,"%alias%",""),"%aliasmain%","")
        ...
        s=Tables(MainTbl).alias & "." & Fields(i)("ColName")
        If Fields(i).exists("ColInfo") Then
          If Fields(i)("ColInfo").ColType="CHAR" and Fields(i)("ColInfo").FixedLength Then s="rtrim(" & s & ") as rico_col" & i
        End If
  ...
  response.write "<span id='" & gridID & "_timer' class='ricoSessionTimer'></span>"
  ...
  response.write vbLf & "Rico.acceptLanguage('" & Request.ServerVariables("HTTP_ACCEPT_LANGUAGE") & "');"
  ...
  if IsArray(s) then
    ...
    select case vartype(s)
      case 8,129,130,200,202
        ' string
        FormatOption="""" & replace(s,"""","\""") & """"
      case 11
        ' boolean
        if s then FormatOption="true" else FormatOption="false"
      case 7,133,134,135:
        ' date/time, format as ISO8601
        FormatOption="'" & year(s) & "-" & PadNumber(month(s),2) & "-" & PadNumber(day(s),2) & "T" & PadNumber(hour(s),2) & ":" & PadNumber(minute(s),2) & ":" & PadNumber(second(s),2) & "'"
      case 4,5,14 
        'single, double, decimal variants. Changing ',' to '.' 
        FormatOption=replace(CStr(s),",",".") 
      case else
        FormatOption=s
    end select
  end if
...
Public function InitScript()
  InitScript = vbLf & bufferVar & "=new Rico.Buffer.AjaxSQL('" & options("XMLprovider") & "', {TimeOut:" & Session.Timeout & "});" & _
               ...
               vbLf & gridVar & ".menu=new Rico.GridMenu();"
  if formView then
    InitScript = InitScript & vbLf & "if(typeof " & gridID & "_FormInit=='function') " & gridID & "_FormInit();" & _
                 vbLf & formVar & "=new Rico.TableEdit(" & gridVar & ");"
  end if
  InitScript = InitScript & vbLf & "if(typeof " & gridID & "_InitComplete=='function') " & gridID & "_InitComplete();"
end function
