...
$clausula="";
 ...
 if(!empty($_GET['periodo1'])&& !empty($_GET['periodo2'])){
	$clausula=$clausula." AND  VN.data_cadastro BETWEEN '".$_GET['periodo1']."' AND '".$_GET['periodo2']."'"; 	
 }
 if(!empty($_GET['data'])){
 	 $clausula=$clausula." AND  VN.data_cadastro like'%$_GET[data]%'"; 	
}
 ...
 //SELECAO DOS DADOS QUE IR�O PRO PDF  
	...
	$sql = $conn->query("SELECT V.gerente_vendas_id,F.nome,count(V.gerente_vendas_id)AS contador ,VN.id
                                          ...
                                          WHERE V.ativo=0 AND VN.ativo=0 AND V.empresa_id=$_GET[emxp]  $clausula GROUP BY V.gerente_vendas_id ");	 
	...
	//echo "SELECT distinct VD.nome_vendedor,VD.id_vendedor from vendedor AS VD INNER JOIN visita AS V ON VD.id_vendedor=V.id_vendedor where VD.ativo=0 AND $gerente $p $d $status order by VD.nome_vendedor"; 
		$vendedores ="SELECT distinct V.vendedor_id,count(V.vendedor_id) AS contador,F.nome,VN.data_cadastro,VN.id,VN.data
                                        FROM visita AS V INNER JOIN funcionario AS F ON V.vendedor_id=F.id 
                                        INNER JOIN vendas AS VN ON V.id=VN.visita_id
                                        INNER JOIN cliente AS C ON C.id=V.cliente_id
                                          INNER JOIN municipio AS M ON C.municipio_codigo=M.id
                                          INNER JOIN estado AS E ON E.id=M.estado_uf
                                         WHERE V.ativo=0 AND VN.ativo=0 AND V.gerente_vendas_id=$row->gerente_vendas_id $clausula GROUP BY VN.data_cadastro ORDER BY F.nome"; 
					
		$sql1 = $conn->query($vendedores);		
...
//gere a A LISTA DE CLIENTE POR VENDEDOR				
            $clientes ="SELECT distinct C.id AS cli_id,C.nome,C.fone1,C.logradouro,F.nome AS funcionario,P.nome AS produto,P.valor,V.local_prospectado,VN.data_cadastro,VN.id
                        FROM  visita AS V INNER JOIN vendas AS VN ON V.id=VN.visita_id 
                            INNER JOIN visita_produto AS VP ON VP.visita_id=V.id
                             ...
                             INNER JOIN cliente AS C ON C.id=V.cliente_id
                             WHERE C.ativo=0 AND VN.ativo=0 AND V.ativo=0 AND P.ativo=0 AND V.vendedor_id=$row1->vendedor_id AND VN.data='$row1->data' AND C.empresa_id=$_GET[emxp]"; 
				
				$sql2 = $conn->query($clientes);	            
   //FIM DA CONSULTA PELO CLIENTE         
				$dados3 = "Vendedor: ".$row1->nome." - Quantidade:".$row1->contador." - Data venda: ".formata_data($row1->data_cadastro);
	  		...
	  		//	$pdf->Rect(10,$y1,173,$l);  
				...
				$pdf->SetY($y1);  
	  		 	$pdf->SetX(10);  
	  		  	$pdf->SetFont('Arial','',9);
	  		 	$pdf->MultiCell(20,6,"Id venda",1,2,1); // ESTA � A CELULA QUE PODE TER DADOS EM MAIS DE UMA LINHA  
				...
				$pdf->SetY($y1);  
	  		 	$pdf->SetX(27);  
	  		  	$pdf->SetFont('Arial','',9);
	  		 	$pdf->MultiCell(74,6,"Cliente",1,2,1); // ESTA � A CELULA QUE PODE TER DADOS EM MAIS DE UMA LINHA  
				...
				$pdf->SetY($y1);  
	  		 	$pdf->SetX(118);  
	  		  	$pdf->SetFont('Arial','',9);
	  		 	$pdf->MultiCell(30,6,"Produto",1,2,1); 
				...
				$pdf->SetY($y1);  
	  		 	$pdf->SetX(146);  
	  		  	$pdf->SetFont('Arial','',9);
	  		 	$pdf->MultiCell(25,6,"Valor",1,2,1);
				...
				$pdf->SetY($y1);  
	  		 	$pdf->SetX(161);  
	  		  	$pdf->SetFont('Arial','',9);
	  		 	$pdf->MultiCell(39,6,"Local",1,2,1);
				...
				if ($y1>=270){
       			...
       			while($row2 =$sql2 ->fetch(PDO::FETCH_OBJ)){
    				//if($_GET['opc']=="vi"){
						...
						$dados6 = $row2->fone1;
						$dados7 =  utf8_decode($row2->produto);
                                                $dados8 = $row2->valor;
                                                $dados9 =$row2->local_prospectado  ;
						//$dados10 =formata_data($row2->data_cadastro);
						$dados0 =$row2->id;	
						
						
					
						//CELULA CONTENDO O id venda
						$pdf->SetY($y1);  
	  				 	$pdf->SetX(10);  
	  					//$pdf->Rect(10,$y,173,$l);  
						...
						//$pdf->Rect(50,$y,173,$l);  
	  				 	$pdf->MultiCell(30,6,$dados6,0,2); // ESTA � A CELULA QUE PODE TER DADOS EM MAIS DE UMA LINHA  
	  					$pdf->SetFont('Arial','',6);  
					
						//CELULA valor PRODUTO
						$pdf->SetY($y1);  
	  				 	$pdf->SetX(118);  
						...
						//CELULA CONTENDO A DATA DA venda 
						$pdf->SetY($y1);  
	  				 	$pdf->SetX(146);  
	  					//$pdf->Rect(50,$y,173,$l);  
	  				 	$pdf->MultiCell(25,6,$dados8,0,2); // ESTA � A CELULA QUE PODE TER DADOS EM MAIS DE UMA LINHA  
	  					$pdf->SetFont('Arial','',6);  
						
                                                $pdf->SetY($y1);  
	  				 	$pdf->SetX(161);  
	  					//$pdf->Rect(50,$y,173,$l);  
	  				 	$pdf->MultiCell(39,6,$dados9,0,2); // ESTA � A CELULA QUE PODE TER DADOS EM MAIS DE UMA LINHA  
	  					$pdf->SetFont('Arial','',6);  
